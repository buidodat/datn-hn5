                ...
                @php
                    ...
                    $validMoviesForDate = collect($date['showtimes'])->filter(function($showtimes) {
                        return $showtimes->filter(function ($showtime) {
                            return \Carbon\Carbon::parse($showtime['start_time'], 'Asia/Ho_Chi_Minh')
                                ->isAfter(\Carbon\Carbon::now('Asia/Ho_Chi_Minh'));
                        })->isNotEmpty();
                    });
                @endphp
                    ...
                    @if ($validShowtimes->isNotEmpty() && $validShowtimes->first() && $validShowtimes->first()->movie)
                        <div class="col-xl-8 col-lg-12 col-md-12 col-sm-12 col-8" style="margin-bottom: 30px">
                            ...
                            <div class="movie-detail-content">
                                <h1 class="movie-title">{{ $validShowtimes->first()->movie->name }}</h1>
                                <ul class="movie-info">
                                    <li><strong>Thể loại:</strong> {{ $validShowtimes->first()->movie->category }}</li>
                                    <li><strong>Thời lượng:</strong> {{ $validShowtimes->first()->movie->duration }} phút</li>
                                </ul>
                                <!-- Lịch chiếu phim -->
                                <div class="showtime-section">
                                    @foreach ($validShowtimes->groupBy('format') as $format => $times)
